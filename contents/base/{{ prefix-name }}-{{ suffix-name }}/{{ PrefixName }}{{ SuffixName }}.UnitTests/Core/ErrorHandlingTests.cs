...
public class ErrorHandlingTests
{
    ...
    {
        // Arrange
        var request = new Get{{ PrefixName }}Request { Id = Guid.NewGuid().ToString() };
        ...
        // Act & Assert
        var exception = await Assert.ThrowsAsync<EntityNotFoundException>(() => _service.Get{{ PrefixName }}(request));
    ...
    {
        // Arrange
        var request = new Get{{ PrefixName }}Request { Id = "invalid-guid" };
        ...
        // Act & Assert
        var exception = await Assert.ThrowsAsync<ValidationException>(() => _service.Get{{ PrefixName }}(request));
        exception.ErrorCode.Should().Be("VALIDATION_ERROR");
    ...
    {
        // Arrange
        var request = new Delete{{ PrefixName }}Request { Id = Guid.NewGuid().ToString() };
        var entityId = Guid.NewGuid();
        ...
        // Act & Assert
        var exception = await Assert.ThrowsAsync<EntityNotFoundException>(() => _service.Delete{{ PrefixName }}(request));
        exception.ErrorCode.Should().Be("ENTITY_NOT_FOUND");
        exception.EntityType.Should().Be("{{ PrefixName }}");
        exception.EntityId.Should().Be(entityId.ToString());
    }
